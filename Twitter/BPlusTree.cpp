...
void BPlusTree::leafNode::insert(int k, int v)
{
    int i = 0;
    for(i = keyNum-1; i >= 0 && key[i] > k; i--) {
        key[i+1] = key[i];
...
bool BPlusTree::get(Node* node, int key, int & v)
{
...
bool BPlusTree::get(int key, int &result)
{
    return get(root, key, result);
}

void BPlusTree::insertNode(Node* node, int key, int v, Node *p, stack<Node*>* parent)
{
    Node* newNode;
    int newKey;
...
void BPlusTree::put(int key, int value)
{
    stack<Node *> parent;
    Node *n = root;
    if(root == NULL) {
...
map<int, int> BPlusTree::getrange(int key1, int key2)
{
    Node *n = root;
    int i = 0;
    map<int, int> res;
